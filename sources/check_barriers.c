...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/03/22 23:56:23 by ael-mejd          #+#    #+#             */
/*   Updated: 2025/03/22 23:56:25 by ael-mejd         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int check_barriers(t_data *data, float x, float y)
{
    ...
    px = (new_x / TILE_SIZE) - ((int)new_x % TILE_SIZE == 0 && data->player.pos_x > new_x);
    py = (new_y / TILE_SIZE) - ((int)new_y % TILE_SIZE == 0 && data->player.pos_y > new_y);
    if (!((int)py < data->map_height && (int)py >= 0 && (int)px >= 0 && (int)px < (int)ft_strlen(data->map[(int)py])))
        return (1);
